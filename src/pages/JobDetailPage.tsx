...
import { Loader2, MapPin, DollarSign, Clock, Users, Send, CheckCircle, XCircle, MessageSquare, AlertTriangle } from "lucide-react";
...
export default function JobDetailPage() {
  ...
  if (isLoading) return <AppLayout><div className="p-6"><CardSkeleton /><CardSkeleton /></div></AppLayout>;
      ...
      <div className="mx-auto max-w-4xl p-6 lg:p-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            <Card>
              <CardContent className="pt-6">
                <h2 className="mb-3 text-lg font-heading font-semibold">Description</h2>
            ...
            {isOwner && applications && applications.length > 0 && (
              <Card>
                <CardHeader><CardTitle className="font-heading">Applications ({applications.length})</CardTitle></CardHeader>
                    ...
                    return (
                      <div key={app.id} className="flex items-start gap-4 rounded-lg border p-4">
                        <Avatar>
                          ...
                          <AvatarFallback className="bg-primary/10 text-primary">{worker?.name?.charAt(0) ?? "?"}</AvatarFallback>
                        </Avatar>
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-sm font-medium text-primary mt-1">${Number(app.offer_price)}</p>
                        </div>
                        {app.status === "pending" && job.status === "open" && (
                          <div className="flex gap-2">
                            <Button size="sm" onClick={() => acceptApplication.mutate(app.id)}>Accept</Button>
                            <Button size="sm" variant="outline" onClick={() => navigate(`/jobs/${id}/chat?with=${app.worker_id}`)}>
            ...
            {job.status === "completed" && (isOwner || isSelectedWorker) && (
              <Card>
                <CardHeader><CardTitle className="font-heading">Leave a Review</CardTitle></CardHeader>
                <CardContent className="space-y-4">
                  <StarRating rating={reviewRating} interactive onChange={setReviewRating} size={24} />
                  <Textarea value={reviewComment} onChange={(e) => setReviewComment(e.target.value)} placeholder="Share your experience (min 10 chars)..." rows={3} />
                  <Button onClick={() => submitReview.mutate()} disabled={reviewRating === 0 || reviewComment.length < 10 || submitReview.isPending}>
          ...
          <div className="space-y-4">
            ...
            {activeRole === "worker" && job.status === "open" && !myApplication && !isOwner && (
              <Sheet open={applyOpen} onOpenChange={setApplyOpen}>
                <SheetTrigger asChild><Button className="w-full h-11"><Send className="mr-2 h-4 w-4" /> Apply Now</Button></SheetTrigger>
                <SheetContent side="bottom" className="h-[60vh]">
                  <SheetHeader><SheetTitle className="font-heading">Apply to this job</SheetTitle></SheetHeader>
                  <div className="space-y-4 mt-6">
                    <div className="space-y-2">
                      <Label>Your offer ($)</Label>
                      <Input type="number" value={applyPrice} onChange={(e) => setApplyPrice(e.target.value)} placeholder="100" />
                    </div>
                    <div className="space-y-2">
                      <Label>Cover message (min 10 chars)</Label>
                      <Textarea value={applyMessage} onChange={(e) => setApplyMessage(e.target.value)} placeholder="Tell the customer why you're the best fit..." rows={4} />
                    </div>
                    <Button className="w-full h-11" onClick={() => submitApplication.mutate()} disabled={submitApplication.isPending || !applyPrice || applyMessage.length < 10}>
            ...
            {myApplication && (
              <Card><CardContent className="pt-6 text-center">
                <p className="text-sm text-muted-foreground">You applied</p>
                <StatusBadge status={myApplication.status} type="application" className="mt-1" />
              </CardContent></Card>
            )}

            {isOwner && job.status === "open" && (
              <Button variant="outline" className="w-full" onClick={() => setConfirmDialog({ open: true, action: "cancelled", title: "Cancel this job?" })}>Cancel Job</Button>
            )}

            {isSelectedWorker && job.status === "assigned" && (
              <div className="space-y-2">
                <Button className="w-full" onClick={() => updateJobStatus.mutate("in_progress")}>Accept & Start Work</Button>
                <Button variant="outline" className="w-full" onClick={() => updateJobStatus.mutate("open")}>Decline</Button>
              </div>
            )}

            {isSelectedWorker && job.status === "in_progress" && (
              <Button className="w-full" onClick={() => setConfirmDialog({ open: true, action: "pending_review", title: "Mark job as complete?" })}>
            ...
            {isOwner && job.status === "pending_review" && (
              <div className="space-y-2">
                <Button className="w-full" onClick={() => updateJobStatus.mutate("completed")}><CheckCircle className="mr-2 h-4 w-4" /> Confirm Completion</Button>
                <Button variant="destructive" className="w-full" onClick={() => updateJobStatus.mutate("disputed")}><AlertTriangle className="mr-2 h-4 w-4" /> Raise Dispute</Button>
              </div>
            )}
        ...
        <Dialog open={confirmDialog.open} onOpenChange={(open) => setConfirmDialog({ ...confirmDialog, open })}>
          <DialogContent>
            ...
            <DialogFooter>
              <Button variant="outline" onClick={() => setConfirmDialog({ open: false, action: "", title: "" })}>Cancel</Button>
              <Button onClick={() => updateJobStatus.mutate(confirmDialog.action)} disabled={updateJobStatus.isPending}>
